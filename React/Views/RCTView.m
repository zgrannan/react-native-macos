...
- (void)react_updateClippedSubviewsWithClipRect:(CGRect)clipRect relativeToView:(RCTPlatformView *)clipView
{
  ...
  if (UIViewSetClipsToBounds(self) && self.subviews.count > 0) {
...
{
  RCTPlatformView *testView = self;
  RCTPlatformView *clipView = nil;
  CGRect clipRect = self.bounds;
  // We will only look for a clipping view up the view hierarchy until we hit the root view.
  while (testView) {
    if (UIViewSetClipsToBounds(testView)) {
      if (clipView) {
...
- (RCTPlatformView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
  ...
  RCTPlatformView *hitSubview = nil;
  ...
  if (needsHitSubview && (![self clipsToBounds] || isPointInside)) {
    ...
    for (UIView *subview in [sortedSubviews reverseObjectEnumerator]) {
      ...
      pointForHitTest = [subview convertPoint:point fromView:self];
#endif
      hitSubview = UIViewHitTestWithEvent(subview, pointForHitTest, event);
  ...
  RCTPlatformView *hitView = (isPointInside ? self : nil);
...
- (void)updateClippedSubviews
{
  ...
  RCTPlatformView *clipView = [self react_findClipView];
  if (clipView) {
...
#if TARGET_OS_OSX
+ (BOOL)viewHasDescendantOverridingDefaultFocusLoop:(RCTPlatformView *)root
{
  return UIViewHasDescendantPassingPredicate(root, ^BOOL(NSView *view) {
    return [view isMemberOfClass:[RCTView class]] && [(RCTView *) view overrideDefaultFocusLoop];
  });
}
#endif
...
{
  if ([self.window autorecalculatesKeyViewLoop]) {
    return;
  }
  if ([[self superview] isKindOfClass:[RCTRootContentView class]] && [RCTView viewHasDescendantOverridingDefaultFocusLoop:self] == NO) {
    NSArray *keyViewLoop = UIViewCalculateKeyViewLoop(self);
    RCTPlatformView *previousKeyView = self;  // The container view is set as the initial previousKeyView
    
    id firstResponder = nil;
    for (NSView *view in keyViewLoop) {
      previousKeyView.nextKeyView = view;
      previousKeyView = view;
      if (firstResponder == nil && [view acceptsFirstResponder]) {
        firstResponder = view;
      }
    }
    if (previousKeyView != self) {
      previousKeyView.nextKeyView = self; // The last view in the loop sets nextKeyView to the container view
    }
    
    if (firstResponder == nil && [keyViewLoop count] > 0) {
      firstResponder = self;
    }
    
    id windowFirstResponder = [[self window] firstResponder];
    if (!windowFirstResponder ||
        windowFirstResponder == self.window ||
        [windowFirstResponder isKindOfClass:[RCTRootView class]]) {
...
{
  return ([self acceptsKeyboardFocus] || [super acceptsFirstResponder]);
}
